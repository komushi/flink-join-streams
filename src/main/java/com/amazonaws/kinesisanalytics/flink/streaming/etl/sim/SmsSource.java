...
public class SmsSource implements SourceFunction<Row> {
    ...
    private SmsSource(int numOfCars, int interval, int flashback, int delay, Boolean fastfoward) {
        intervalInMs = interval;
        delayInMs = delay;
        crtTime = System.currentTimeMillis() - flashback - 2;
    ...
    public static SmsSource create(int cars, int interval, int flashback, int delay, Boolean fastfoward) {
        return new SmsSource(cars, interval, flashback, delay, fastfoward);
    }
